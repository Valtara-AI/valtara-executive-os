...
VEX-OS-CLM-001 · Version 1.0 · August 2026

| Document ID | VEX-OS-CLM-001 |
| --- | --- |
| Version | 1.0 |
...
This document (VEX-OS-CLM-001) serves two purposes: (1) it is the governance record of the CLAUDE.md file, capturing its rationale and content specification; (2) the actual CLAUDE.md markdown file in the repository root is the working artifact that Claude Code reads. Both must be kept in sync.

CLAUDE.md is the last document initialized in the VEX-OS pre-build suite. It synthesizes the entire documentation corpus into a single context file that gives Claude Code precise, grounded instructions for building this system. It replaces tribal knowledge with documented constraint.
...
VEX-OS is a domain-agnostic, AI-powered Executive Operating System.
...
docs/01-technical/VEX-OS-API-001 Section 6. No undocumented MCP connections.
...
VEX-OS-ETF-001  — Ethical constraints (prohibited behaviors)
...
The docx governance record (VEX-OS-CLM-001) must be updated in parallel with the CLAUDE.md markdown file. Both are kept in sync at all times.
...
*VEX-OS-CLM-001 · Version 1.0 · August 2026 · Internal — Committed to Repository*
