...
VEX-OS-AMC-001 · Version 1.0 · August 2026

| Document ID | VEX-OS-AMC-001 |
| --- | --- |
| Version | 1.0 |
...
This AI Model Card documents the selection rationale, intended use, operational scope, limitations, and governance controls for the AI models deployed in Valtara Executive OS (VEX-OS). It follows the Model Card framework (Mitchell et al., 2019) adapted for a multi-model, provider-agnostic deployment context.

VEX-OS does not depend on any single AI model or provider. All models are accessed via a provider-agnostic adapter layer. This document governs the selection criteria and operational parameters that apply regardless of which provider is configured.

# 2. Model Deployment Strategy

**2.1 Multi-Model Architecture**

VEX-OS deploys different models for different task types, balancing cost, capability, and latency requirements. No single model handles all tasks.
...
Any LLM provider configured in VEX-OS must meet the following criteria before deployment:
...
- Medical, legal, or financial advice as a primary service — VEX-OS may assist executives in those fields with their own work, but does not provide professional advice as a product
- Autonomous execution of financial transactions — VEX-OS agents cannot initiate payments or transfers
...
| Limitation | Mitigation in VEX-OS |
| --- | --- |
...
| Bias in generated content | All outputs reviewed by executive before use; bias assessment included in periodic output quality review; documented in VEX-OS-ETF-001 |
...
*VEX-OS-AMC-001 · Version 1.0 · August 2026 · Confidential — Internal Use Only*
