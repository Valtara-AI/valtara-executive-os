...
VEX-OS-API-001 · Version 1.0 · August 2026

| Document ID | VEX-OS-API-001 |
| --- | --- |
| Version | 1.0 |
...
This API & Integration Specification defines the internal VEX-OS API contract and the specifications for all external integrations. It is the authoritative reference for every interface in the system: the REST API consumed by the frontend, the integration adapter contracts for email and calendar providers, and the LLM provider adapter interface.

VEX-OS follows an API-first integration policy. All third-party integrations use published REST APIs. MCP connections are permitted only where no REST API equivalent exists, and each such exception must be documented in Section 6 of this document with explicit justification.
...
| Data minimization | Only retrieve emails relevant to active agent task context; no bulk historical download to VEX-OS storage |
...
The following is the canonical .env template for VEX-OS. The .env.example file in the repository contains this structure with no real values. The actual .env file is always listed in .gitignore and must never be committed.
...
*VEX-OS-API-001 · Version 1.0 · August 2026 · Confidential — Internal Use Only*
