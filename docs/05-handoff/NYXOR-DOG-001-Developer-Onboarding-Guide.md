...
NYXOR-DOG-001 · Version 1.0 · August 2026

| Document ID    | NYXOR-DOG-001                                          |
| -------------- | ------------------------------------------------------ |
| Version        | 1.0                                                    |
| Status         | Active                                                 |
| Owner          | Francis Ogbogu — Chief AI Officer, Valtara AI          |
| Audience       | New developers joining the NYXOR build                 |
| Date           | August 2026                                            |
...
2. NYXOR-SRS-001 (docs/01-technical/) — system behavior specification

3. NYXOR-DL-001 (docs/04-build-governance/) — decision log; understand why key decisions were made
...
DL-SEC-004: your `.env` needs both `DATABASE_MIGRATION_URL` (the privileged docker-compose connection, `postgresql://nyxor:nyxor_local_dev@localhost:5432/nyxor`) and a `DB_APP_ROLE_PASSWORD` of your choosing _before_ running this. This step both applies pending migrations and provisions/updates the restricted `nyxor_app` role the application actually connects as — after it completes, set `DATABASE_URL` to `postgresql://nyxor_app:<your DB_APP_ROLE_PASSWORD>@localhost:5432/nyxor` and re-run this step if you change the password later. See `.env.example` for the full explanation.
...
1. Check the Decision Log (NYXOR-DL-001) — has this been decided already?

2. Create a feature branch: git checkout -b feature/NYXOR-[ticket]-[description]
...
_NYXOR-DOG-001 · Version 1.0 · August 2026 · Confidential — Internal Use Only_
