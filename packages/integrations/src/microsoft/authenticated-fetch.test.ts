...
// Mirrors google/authenticated-fetch.test.ts exactly - see that file for
// why fetch is mocked rather than requiring real Microsoft credentials.
...
describe.skipIf(!hasDb)("graphApiFetch", () => {
  ...
  async function makeConnectedExecutive(overrides?: { expiresAt?: Date; refreshToken?: string }) {
    ...
    const [executive] = await db
      ...
      .values({
        name: "Graph Fetch Test Exec",
        email: `graph-fetch-test-${Date.now()}-${Math.random()}@example.com`,
      })
      .returning();
  ...
  it("throws IntegrationNotConnectedError when no tokens are stored", async () => {
    ...
    const [executive] = await db
      .insert(schema.executives)
      .values({ name: "No Tokens", email: `no-tokens-ms-${Date.now()}@example.com` })
      .returning();
    cleanupExecutiveIds.push(executive!.id);

    await expect(
      graphApiFetch(executive!.id, "https://graph.microsoft.com/v1.0/me"),
  ...
  it("succeeds on the first attempt with a valid token, no refresh needed", async () => {
    ...
    await graphApiFetch(executive.id, "https://graph.microsoft.com/v1.0/me");
  ...
  it("retries once with a refreshed token on a 401", async () => {
    ...
    const res = await graphApiFetch(executive.id, "https://graph.microsoft.com/v1.0/me");
