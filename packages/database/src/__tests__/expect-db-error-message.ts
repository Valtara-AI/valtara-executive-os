...
// Postgres error (the trigger/permission message these tests actually
// care about) is preserved on .cause instead (drizzle-orm/errors.ts).
// A plain `.rejects.toThrow(pattern)` only checks .message, so it stopped
// matching once this codebase bumped past 0.38 (DL-SEC-005) - this helper
// unwraps .cause first.
